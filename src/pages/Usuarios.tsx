...
import { listProfiles, updateProfileRole } from "../lib/supabaseRepository";
...
export function UsuariosAtivos() {
  ...
  return <Card><div className="mb-5 flex items-start gap-3"><div className="rounded-lg bg-primary-soft p-3 text-primary"><Users className="h-5 w-5" /></div><div><h2 className="text-2xl font-bold">Usuários ativos</h2><p className="mt-1 text-muted">Perfis com acesso ao laboratório e seus papéis atuais.</p></div></div>
    ...
    <div className="grid gap-3">{profiles.map((user) => <div className="rounded-lg border border-border bg-background p-4" key={user.id}><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-lg font-bold">{user.full_name}</p><p className="text-sm text-muted">{user.email}</p></div><StatusBadge label={user.role === "coordinator" ? "Coordenador" : "Pesquisador"} variant={user.role === "coordinator" ? "success" : "neutral"} /></div>
      <div className="mt-4">{user.role === "researcher" ? <Button className="min-h-9 px-3 py-2 text-sm" disabled={savingId === user.id} onClick={() => changeRole(user.id, "coordinator")}><ArrowUpRight className="mr-2 h-4 w-4" />Promover a coordenador</Button> : user.id !== profile?.id ? <Button className="min-h-9 px-3 py-2 text-sm" variant="secondary" disabled={savingId === user.id} onClick={() => changeRole(user.id, "researcher")}><ArrowUpRight className="mr-2 h-4 w-4 rotate-180" />Alterar para pesquisador</Button> : null}</div></div>)}</div>
  </Card>;
}
